...
class BD {
    ...
    function conectar() {
        ...
        $this->link = mysql_connect("localhost", "root", "ab9843zqab");
        if (!$this->link) {
            return false;
            exit;
        }
        if (!mysql_select_db("eltiti")) {
            return false;
            exit;
        } else {
            return true;
...
function MostrarErrorBD($a) {
    echo $_error_a_mostrar = $a;
    //include("adm/plantillas/completo_error.php");
}
